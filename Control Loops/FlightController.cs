...
namespace AscentProfiler
{
        ...
        public class FlightController
        {
                ...
                //instantiate autopilot and attitude controller
                //send commands to attitude
                //send commands for throttle/thrust
                // engageAutopilot method
                // disengageAutopilot method
        ...
        public static class AttitudeController0
        {
                ...
                public static bool setval;
                public static Vessel vessel;

                public static void setAttitudeHold(double heading, double pitch, double roll)
                {

                        //attitudeTarget = Quaternion.AngleAxis((float)heading, Vector3.up) * Quaternion.AngleAxis(-(float)pitch, Vector3.right) * Quaternion.AngleAxis(-(float)roll, Vector3.forward);

                }


                public static void HoldAttitude(Quaternion target, FlightCtrlState s, Vessel v)
                {
                        ...
                        if (!setval)
                        {
                                ...
                                v.OnFlyByWire += new FlightInputCallback(AttitudeDriver);
                ...
                public static void AttitudeDriver(FlightCtrlState s)
                {
                        //kOS.SteeringHelper.KillRotation(s, vessel);

                        kOS.SteeringHelper.SteerShipToward(attitudeTarget, s, vessel);
...
namespace kOS
{
        ...
        public static class SteeringHelper
        {
                ...
                public static Vector3d GetEffectiveInertia(Vessel vessel, Vector3d torque)
                {
                        ...
                        var retVar = Vector3d.Scale(Sign(angularMomentum) * 2.0f, Vector3d.Scale(Pow(angularMomentum, 2), Inverse(Vector3d.Scale(torque, momentOfInertia))));
